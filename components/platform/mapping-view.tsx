...
import type { MappingDraftInput, MappingRule as ApiMappingRule, MappingVersion, SceneReadiness, TransformConfig } from '@outbound/contracts';
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle } from '@/components/ui/dialog';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { loadMappingCenter, PlatformApiError, publishMappings, removeMappingDraft, requestVariableSync, saveMappingDraft, type MappingDraftRecord } from '@/lib/platform-api';
...
type Scene = { id: string; name: string; robotDefId: string; companies: string; status: SceneStatus; coverage: number; expected: number; lastSync: string; lastSyncAt: string | null; missingVariables: string[]; variables: string[]; issue: string };
...
const sampleInputs: Record<string, string> = {
  婚期: '2026/10/18', 套餐意向: 'A', 门店名称: '上海总店', 顾问姓名: '陈顾问',
  老客权益: '周年加片', 客户来源: '婚博会', 活动名称: '秋季档期', 预算范围: '8000-12000', 礼服风格: '法式轻盈', 客户等级: 'VIP',
};
...
function formatDateTime(value: string | null, short = false) {
  ...
  const parts = new Intl.DateTimeFormat('zh-CN', {
    timeZone: 'Asia/Shanghai', year: short ? undefined : 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false,
  }).formatToParts(date).reduce<Record<string, string>>((result, part) => ({ ...result, [part.type]: part.value }), {});
  return short ? `${parts.month}-${parts.day} ${parts.hour}:${parts.minute}` : `${parts.year}-${parts.month}-${parts.day} ${parts.hour}:${parts.minute}`;
}

function transformConfigToText(config: TransformConfig) {
  if (config.type === 'TEXT') return ({ TRIM: '去除首尾空格', PRESERVE: '保持原值', UPPERCASE: '转为大写', LOWERCASE: '转为小写' } as const)[config.mode];
  if (config.type === 'DATE') return config.outputFormat;
  if (config.type === 'MONEY') return `${config.inputUnit === 'CENT' ? '分转元' : '元'} · 保留${config.decimalPlaces}位`;
  if (config.type === 'ENUM') return Object.entries(config.values).map(([key, value]) => `${key}=${value}`).join('\n');
...
function draftToApiInput(draft: MappingDraft): MappingDraftInput {
  ...
  if (draft.transform === 'TEXT') {
    const modes = { 去除首尾空格: 'TRIM', 保持原值: 'PRESERVE', 转为大写: 'UPPERCASE', 转为小写: 'LOWERCASE' } as const;
    transformConfig = { type: 'TEXT', mode: modes[draft.transformConfig as keyof typeof modes] ?? 'TRIM' };
  } else if (draft.transform === 'DATE') {
    transformConfig = { type: 'DATE', outputFormat: draft.transformConfig as 'YYYY-MM-DD' | 'YYYY年MM月DD日' | 'MM/DD/YYYY' };
  } else if (draft.transform === 'MONEY') {
    transformConfig = { type: 'MONEY', inputUnit: draft.transformConfig.includes('分转元') ? 'CENT' : 'YUAN', decimalPlaces: draft.transformConfig.includes('2位') ? 2 : 0 };
  } else if (draft.transform === 'ENUM') {
    const values = Object.fromEntries(draft.transformConfig.split('\n').map((line) => line.split('=')).filter(([key, value]) => key?.trim() && value?.trim()).map(([key, value]) => [key.trim(), value.trim()]));
    transformConfig = { type: 'ENUM', values };
  } else {
    transformConfig = { type: 'TEMPLATE', template: draft.transformConfig || '{{value}}' };
  }
  return {
    ...
    defaultValue: draft.emptyPolicy === 'DEFAULT' ? draft.defaultValue || null : null,
  };
}
...
function apiRuleToView(rule: ApiMappingRule): MappingRule {
  ...
  return { id: rule.id, ...draft, status: rule.status, version: rule.version, sampleOutput: applyPreview(draft) };
}

function apiDraftToView(draft: MappingDraftRecord, rules: MappingRule[]): MappingDraft {
  const existing = rules.find((rule) => rule.variable === draft.baiyingVariableName);
  if (draft.changeType === 'REMOVE') return existing ? { ...existing, remove: true } : { ...defaultDraft(draft.baiyingVariableName), remove: true };
  const config = draft.transformConfig ?? { type: 'TEXT' as const, mode: 'TRIM' as const };
  return {
...
function apiVersionToView(version: MappingVersion): VersionRecord {
  return { version: version.version, publishedAt: formatDateTime(version.publishedAt), publisher: version.publisherId, ruleCount: version.ruleCount, change: version.changeSummary };
}
  ...
  for (const scene of scenes) {
    for (const variable of scene.missingVariables) {
      ...
      issueByVariable.set(variable, {
        ...
        note: drift ? '不同公司返回的变量集合不一致，需要复核。' : '百应话术变量接口已返回该变量，尚无已发布映射。',
      });
    }
  }
...
export function MappingView() {
  ...
  const [ruleStatus, setRuleStatus] = useState<'ALL' | MappingDisplayStatus>('ALL');
  ...
  const refreshData = useCallback(async () => {
    try {
      ...
      const message = error instanceof PlatformApiError ? error.message : '字段映射数据加载失败';
      setFeedback(message);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => { queueMicrotask(() => void refreshData()); }, [refreshData]);

  const issues = useMemo(() => deriveIssues(scenes), [scenes]);
  const currentVersion = versions[0]?.version ?? 0;
  const latestSyncAt = scenes.map((scene) => scene.lastSyncAt).filter((value): value is string => Boolean(value)).sort().at(-1) ?? null;
  const lastSync = formatDateTime(latestSyncAt, true);
  const nextSync = latestSyncAt ? formatDateTime(new Date(new Date(latestSyncAt).getTime() + 6 * 60 * 60 * 1000).toISOString(), true) : '—';
  const companyCoverage = Math.max(0, ...scenes.map((scene) => Number.parseInt(scene.companies, 10) || 0));
  ...
  const syncedVariables = useMemo(() => [...new Set(scenes.flatMap((scene) => scene.variables))].sort((left, right) => left.localeCompare(right, 'zh-CN')), [scenes]);
  const sortedScenes = useMemo(() => [...scenes].sort((left, right) => left.name.localeCompare(right.name, 'zh-CN')), [scenes]);
  const mappingScene = mappingSceneId === 'ALL' ? null : scenes.find((scene) => scene.id === mappingSceneId) ?? null;
  const mappingVariables = useMemo(() => mappingScene
    ? [...new Set(mappingScene.variables)].sort((left, right) => left.localeCompare(right, 'zh-CN'))
    : syncedVariables, [mappingScene, syncedVariables]);
  const variableScenes = useMemo(() => {
    const result = new Map<string, Scene[]>();
    for (const scene of scenes) {
      for (const variable of new Set(scene.variables)) result.set(variable, [...(result.get(variable) ?? []), scene]);
    }
    return result;
  }, [scenes]);
  const mappedVariableCount = mappingVariables.filter((variable) => rules.some((rule) => rule.variable === variable && rule.status === 'PUBLISHED')).length;
  const ruleRows = useMemo(() => {
    const merged = mappingVariables.map((variable, index) => {
      ...
      if (rule) return draft ? { ...rule, ...draft, sampleOutput: applyPreview(draft), displayStatus: draft.remove ? 'REMOVED' as const : 'DRAFT' as const } : { ...rule, displayStatus: rule.status };
      if (draft) return { id: `draft-${index}`, ...draft, status: 'PUBLISHED' as const, version: currentVersion + 1, sampleOutput: applyPreview(draft), displayStatus: 'DRAFT' as const };
      const unmapped = { ...defaultDraft(variable), sampleInput: sampleInputs[variable] ?? '' };
      return { id: `unmapped-${index}`, ...unmapped, status: 'PUBLISHED' as const, version: 0, sampleOutput: '', displayStatus: 'UNMAPPED' as const };
    });
  ...
  const saveDraft = async (event: { preventDefault(): void }) => {
    ...
    try {
      ...
      setFeedback(`“${draftForm.variable}”已写入数据库草稿；相关场景仍保持阻断，发布成功后才会重新计算状态。`);
    } catch (error) {
      setFeedback(error instanceof PlatformApiError ? error.message : '映射草稿保存失败');
    } finally {
      setSaving(false);
    }
  };
  const stageRemoval = async (variable: string) => {
    setSaving(true);
    try {
      await removeMappingDraft({ baiyingVariableName: variable, removalReason: '百应最新变量快照已移除该变量' });
      await refreshData();
      setFeedback(`“${variable}”已写入待移除草稿；发布前旧版本仍保持生效。`);
    } catch (error) {
      setFeedback(error instanceof PlatformApiError ? error.message : '标记移除失败');
    } finally {
      setSaving(false);
    }
  };
  const publishDrafts = async () => {
    ...
    try {
      ...
      const result = await publishMappings({ publisherId: 'platform-admin', changeSummary: `发布 ${drafts.length} 项映射变更：${variables}` });
      await refreshData();
      setPublishOpen(false);
      setFeedback(`映射版本 v${result.version.version} 已发布并写入 PostgreSQL。新任务将锁定该版本，已创建任务不受影响。`);
    } catch (error) {
      setFeedback(error instanceof PlatformApiError ? error.message : '映射发布失败');
    } finally {
      setSaving(false);
    }
  };
  const syncVariables = async () => {
    ...
    try {
      ...
      setFeedback(`同步任务已进入队列（${job.jobId.slice(0, 8)}），页面状态将在后台完成百应话术变量同步后更新。`);
    } catch (error) {
      setFeedback(error instanceof PlatformApiError ? error.message : '同步任务创建失败');
    } finally {
      setSyncing(false);
    }
  };
  ...
  return <div className="mapping-center mapping-center-complete">
    <header className="mapping-compact-intro">
      <div><p className="eyebrow">话术变量映射</p><h2>字段映射中心</h2><p>同步百应当前可用话术的自定义变量，并配置变量从 ERP 或 CRM 读取值的字段；同名变量全局共用一套映射。</p></div>
      <button className="primary-button sync-button" onClick={syncVariables} disabled={syncing || loading}><RefreshCcw size={14} className={syncing ? 'spin' : ''} />{syncing ? '正在提交…' : '同步百应变量'}</button>
    </header>
    {feedback ? <output className="notice mapping-feedback"><Check size={14} />{feedback}</output> : null}
    {loading ? <div className="notice mapping-feedback"><RefreshCcw size={14} className="spin" />正在从平台 API 读取字段映射数据…</div> : null}

    <section className="inspection-board" aria-label="变量巡检">
      <div className="inspection-lead"><div><p className="eyebrow">VARIABLE INSPECTION</p><h3>变量巡检</h3><p>每 6 小时自动巡检；失败时保留上次成功快照，超过 24 小时则阻断使用。</p></div><div className="sync-stamp"><span>最后成功同步</span><b>{lastSync}</b><small>下一次自动同步 {nextSync}</small></div></div>
      <div className="inspection-metrics inspection-metrics-six"><article><span>场景总数</span><b>{scenes.length}</b><small>单场景最多覆盖 {companyCoverage} 家公司</small></article>{(['ACTIVE', 'PENDING_MAPPING', 'DRIFT_DETECTED', 'STALE_SYNC', 'DISABLED'] as SceneStatus[]).map((status) => <article className={status === 'ACTIVE' ? 'metric-ok' : status === 'PENDING_MAPPING' ? 'metric-warn' : status === 'DRIFT_DETECTED' ? 'metric-danger' : 'metric-muted'} key={status}><span>{sceneStatusMeta[status].short}</span><b>{counts[status]}</b><small>{status === 'ACTIVE' ? '允许新任务 / 导入' : status === 'DISABLED' ? '管理员停用' : '阻断新任务 / 导入'}</small></article>)}</div>
    </section>

    <section className="mapping-guide mapping-guide-direct" aria-label="字段映射操作说明">
      <div className="guide-copy"><span>映射范围</span><h3>只映射百应返回的自定义变量</h3><p>客户名称和联系方式是百应内置字段，分别直接写入 name、phone；其他自定义变量才配置 ERP / CRM 映射。</p></div>
      <div className="mapping-chain-demo">
        <div className="baiying-node"><span>百应变量接口返回</span><b>婚期</b><small>变量名不可手工新增</small></div><ChevronRight size={17} />
    ...
    <Tabs value={activeTab} onValueChange={setActiveTab} className="mapping-tabs-shell">
      <TabsList variant="line" className="mapping-tabs-list" aria-label="字段映射功能区">
        ...
        <TabsTrigger value="rules" className="mapping-tab-trigger">变量映射 <small>{syncedVariables.length}</small></TabsTrigger>
      ...
      <TabsContent value="issues" className="mapping-tab-panel"><Panel title="待处理变量" meta={`${issues.length} 项需处理`} className="pending-panel">
        ...
        {drafts.length ? <div className="draft-publish-bar"><div><ShieldCheck size={17} /><span><b>{drafts.length} 项变更已保存到数据库草稿</b><small>草稿不会解除场景阻断；发布后生成不可变版本 v{currentVersion + 1}</small></span></div><button className="primary-button" onClick={() => setPublishOpen(true)} disabled={saving}>审核并发布</button></div> : null}
      </Panel></TabsContent>

      <TabsContent value="scenes" className="mapping-tab-panel"><Panel title="话术场景就绪状态" meta="只有 ACTIVE 可创建新任务"><div className="rules-toolbar"><label className="search-box"><Search size={15} /><input value={sceneQuery} onChange={(event) => setSceneQuery(event.target.value)} placeholder="搜索话术名称、话术 ID 或公司" /></label><select className="filter-button" aria-label="场景状态" value={sceneStatus} onChange={(event) => setSceneStatus(event.target.value as 'ALL' | SceneStatus)}><option value="ALL">全部状态</option>{Object.entries(sceneStatusMeta).map(([value, meta]) => <option value={value} key={value}>{meta.short}</option>)}</select></div><div className="table-wrap"><table className="data-table scene-readiness-table"><thead><tr><th>话术场景</th><th>公司覆盖</th><th>变量覆盖</th><th>最后成功同步</th><th>就绪状态</th><th>问题摘要</th><th>操作</th></tr></thead><tbody>{filteredScenes.map((scene) => <tr key={scene.id}><td><b>{scene.name}</b><span className="table-meta">{scene.robotDefId}</span></td><td>{scene.companies}</td><td><b>{scene.coverage} / {scene.expected}</b><div className="coverage-track"><i style={{ width: `${scene.expected ? scene.coverage / scene.expected * 100 : 0}%` }} /></div></td><td>{scene.lastSync}</td><td><Status tone={sceneStatusMeta[scene.status].tone}>{sceneStatusMeta[scene.status].label}</Status></td><td className="scene-issue-cell">{scene.issue || '—'}</td><td><button className="table-action" onClick={() => setSelectedScene(scene)}>查看详情</button></td></tr>)}</tbody></table></div></Panel></TabsContent>

      <TabsContent value="rules" className="mapping-tab-panel"><Panel title="百应话术变量映射" meta={`${mappingScene ? mappingScene.name : `全部 ${scenes.length} 个话术`} · 当前显示 ${mappingVariables.length} 个变量 · 已发布 ${mappedVariableCount} 个`} className="rules-panel"><div className="rules-toolbar mapping-rules-toolbar"><label className="search-box"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索百应变量、ERP 字段或 CRM 字段" /></label><label className="mapping-scene-filter"><span>话术范围</span><select className="filter-button" aria-label="映射话术范围" value={mappingScene?.id ?? 'ALL'} onChange={(event) => setMappingSceneId(event.target.value)}><option value="ALL">全部话术 · 去重 {syncedVariables.length} 个变量</option>{sortedScenes.map((scene) => <option value={scene.id} key={scene.id}>{scene.name} · {scene.variables.length} 个变量</option>)}</select></label><select className="filter-button" aria-label="规则状态" value={ruleStatus} onChange={(event) => setRuleStatus(event.target.value as typeof ruleStatus)}><option value="ALL">全部状态</option><option value="UNMAPPED">待配置</option><option value="PUBLISHED">已发布</option><option value="DRAFT">有草稿</option><option value="REMOVED">已移除</option></select><button className="primary-button" onClick={() => setPublishOpen(true)} disabled={!drafts.length || saving}>发布草稿{drafts.length ? `（${drafts.length}）` : ''}</button></div><div className="rule-scope"><span>{mappingScene ? '单话术核对' : '全部话术 · 全局去重'}</span><p>{mappingScene ? <>当前只显示“{mappingScene.name}”本次接口快照返回的 {mappingVariables.length} 个变量，与场景状态详情口径一致。</> : <>当前显示全部 {scenes.length} 个话术的变量并集，同名变量只保留一行；可在上方选择一个话术，与场景状态逐项核对。</>}</p></div><div className="table-wrap"><table className="data-table mapping-table direct-mapping-table"><thead><tr><th>百应话术变量</th><th>关联话术</th><th><span className="source-header source-erp">ERP 取值字段</span></th><th><span className="source-header source-crm">CRM 取值字段</span></th><th>写入百应</th><th>转换规则</th><th>空值策略</th><th>版本状态</th><th>样例预览</th><th>操作</th></tr></thead><tbody>{ruleRows.map((rule) => {
        const relatedScenes = variableScenes.get(rule.variable) ?? [];
        const relatedSceneNames = relatedScenes.map((scene) => scene.name);
        return <tr key={rule.id}><td><b>{rule.variable}</b><small className="table-meta">百应接口同步</small></td><td><div className="variable-scene-usage" aria-label={`关联话术：${relatedSceneNames.join('、')}`} title={relatedSceneNames.join('、')}><b>{relatedScenes.length} 个话术</b><small>{mappingScene ? mappingScene.name : relatedSceneNames.length > 1 ? `${relatedSceneNames[0]} 等` : relatedSceneNames[0] || '无关联话术'}</small></div></td><td><MappedSourceField source="ERP" value={rule.erpField} /></td><td><MappedSourceField source="CRM" value={rule.crmField} /></td><td><span className="payload-path">properties.{rule.variable}</span></td><td>{rule.displayStatus === 'UNMAPPED' ? '—' : <><b>{rule.transform}</b><small className="table-meta config-summary">{rule.transformConfig}</small></>}</td><td>{rule.displayStatus === 'UNMAPPED' ? '—' : rule.emptyPolicy === 'BLOCK' ? <Status tone="red">缺失阻断</Status> : <div><Status tone="blue">使用默认值</Status><small className="table-meta">{rule.defaultValue}</small></div>}</td><td><Status tone={rule.displayStatus === 'PUBLISHED' ? 'green' : rule.displayStatus === 'REMOVED' ? 'gray' : 'amber'}>{rule.displayStatus === 'PUBLISHED' ? `v${rule.version} 已发布` : rule.displayStatus === 'REMOVED' ? '待移除 / 已移除' : rule.displayStatus === 'DRAFT' ? '草稿待发布' : '待配置'}</Status></td><td>{rule.displayStatus === 'UNMAPPED' ? '—' : <span className="sample-preview"><small>{rule.sampleInput || '空值'}</small>{rule.sampleOutput}</span>}</td><td>{rule.displayStatus === 'REMOVED' ? '—' : <button className="table-action" onClick={() => openMapping(rule.variable)}>{rule.displayStatus === 'UNMAPPED' ? '配置映射' : '编辑映射'}</button>}</td></tr>;
      })}</tbody></table></div>{!ruleRows.length ? <div className="rule-empty">{mappingScene && !mappingVariables.length ? '该话术本次接口快照未返回需要映射的自定义变量' : '没有符合条件的百应变量'}</div> : null}</Panel></TabsContent>
      ...
      <DialogContent className="mapping-dialog mapping-dialog-refined max-w-[760px] gap-0 p-0" showCloseButton={false}>
        <form onSubmit={saveDraft}>
          <DialogHeader className="mapping-dialog-header mapping-dialog-header-refined">
            <div>
              <p className="mapping-api-label"><span>百应变量接口</span> 同步变量映射</p>
    ...
    <Dialog open={Boolean(selectedScene)} onOpenChange={(open) => { if (!open) setSelectedScene(null); }}>
      <DialogContent className="scene-dialog scene-variable-dialog max-w-[820px]" showCloseButton={false}>
        <DialogHeader className="scene-variable-dialog-header">
          <div><p className="eyebrow">SCENE VARIABLE SNAPSHOT</p><DialogTitle>{selectedScene?.name}</DialogTitle><DialogDescription>{selectedScene?.robotDefId} · {selectedScene?.companies} · 百应变量接口最近同步 {selectedScene?.lastSync}</DialogDescription></div>
        ...
        {selectedScene ? <>
          ...
          <div className="scene-variable-summary scene-variable-summary-compact"><div><span>接口返回变量</span><b>{selectedScene.expected}</b></div><div><span>已发布映射</span><b>{selectedScene.coverage}</b></div><div><span>待处理变量</span><b>{selectedScene.expected - selectedScene.coverage}</b></div><div><span>新任务 / 导入</span><b>{selectedScene.status === 'ACTIVE' ? '允许' : '阻断'}</b></div></div>

          <section className="scene-variable-section">
            <div className="scene-variable-section-heading"><div><b>当前话术变量</b><small>以本次百应接口快照为准</small></div><span>{sceneVariableRows.length} 个变量</span></div>
            <div className="table-wrap"><table className="data-table scene-variable-table"><thead><tr><th>百应变量</th><th>接口当前情况</th><th>ERP 字段</th><th>CRM 字段</th><th>映射状态</th><th>操作</th></tr></thead><tbody>{sceneVariableRows.map((row) => <tr key={row.variable}><td><b>{row.variable}</b><small className="table-meta">properties.{row.variable}</small></td><td><span className="api-variable-state">{row.apiStatus}</span></td><td><code className={`scene-source-value scene-source-erp ${row.erpField ? '' : 'is-empty'}`}>{row.erpField || '未配置'}</code></td><td><code className={`scene-source-value scene-source-crm ${row.crmField ? '' : 'is-empty'}`}>{row.crmField || '未配置'}</code></td><td><Status tone={row.tone}>{row.mappingStatus}</Status></td><td><button className="table-action" onClick={() => { setSelectedScene(null); if (row.mappingStatus === '待确认移除') void stageRemoval(row.variable); else openMapping(row.variable); }}>{row.mappingStatus === '待确认移除' ? '确认移除' : row.mappingStatus.includes('已发布') ? '编辑' : '配置'}</button></td></tr>)}</tbody></table></div>
          </section>

          {selectedScene.status === 'DRIFT_DETECTED' ? <div className="drift-compare"><h4>跨公司变量差异</h4><p><span>当前判断</span><code>最新成功快照的变量集合不一致</code></p><p><span>处理建议</span><code>重新同步并核对各公司对应的话术版本</code></p></div> : null}
          <DialogFooter className="scene-dialog-footer scene-variable-dialog-footer"><p>变量状态变化后，需要重新发布映射并同步复核。</p><div className="scene-variable-dialog-actions"><button className="filter-button" onClick={() => setSelectedScene(null)}>关闭</button><button className="primary-button" onClick={() => { setMappingSceneId(selectedScene.id); setSelectedScene(null); setActiveTab('rules'); }}>查看该话术映射</button></div></DialogFooter>
        </> : null}
      </DialogContent>
    </Dialog>
  </div>;
