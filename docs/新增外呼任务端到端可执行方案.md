...
- [x] 话费设置改为数据库版本发布；当前运营入口中的影楼价格和海南阶梯均直接读取数据库，不再读写 `localStorage`。
...
6B-1 完成记录（2026-09-06）：新增真实影楼分页/创建/修改/启停接口、不可变账户流水与线下充值接口，以及客户价格预览/版本发布接口。新增影楼自动生成 `YL-YYYYMM-NNNN` 编号，联系人电话加密保存并脱敏读取；充值在账户行锁内同步写余额、账本和审计，支持幂等重放；统一和单影楼价格均先预览再发布，可立即或次日零点生效，旧任务继续使用原快照。供应商成本阶梯为数据库只读，后续审批子阶段再开放编辑。
