...
阶段 5 尚余阿里云落地项：私有 OSS/SSE-KMS、KMS 密钥提供器、真实百应录音域名允许列表，以及取得 ERP/CRM 测试地址后启用的受控 HTTPS 投递适配器。当前不会把本地对象存储、模拟录音源或零网络接收器接入生产启动路径；这些外部条件未具备前，阶段 5 不标记整体完成。
...
影楼回传端点维护补充记录（2026-09-07）：新增和编辑影楼弹窗均可配置 ERP 结果回传、ERP 录音回传、CRM 结果回传、CRM 录音回传四个 HTTPS 地址。每个来源的结果与录音地址必须成对填写；保存时按来源生成新的不可变 `DRAFT` 端点版本并归档旧草稿，未配置 HMAC/KMS 密钥前不允许进入真实回传链路。任务受理继续冻结当时启用的端点版本，后续修改不会改变历史任务。
...
本地可执行 `npm run db:verify:stage6b2` 验证双人分离、重复请求/重复决策、余额保护、唯一入账、拒绝不入账及嵌套敏感字段脱敏；执行 `npm run stage6b2:verify:ui` 验证审批表单、复核决策入口与真实审计详情。浏览器验收中的待复核申请由只读网络夹具提供，不会产生真实资金写入。当前 `X-Actor-Id` 仅用于本地内部身份联调；阿里云生产环境必须由登录/SSO 网关注入可信操作人，不能信任浏览器自行传入的身份头。
...
本机本轮实测：10,000 条受理 3.662 秒、编排 1.358 秒；普通任务并发受理 P95 1.963 秒；回调写入 10.141 秒、单条写入 P95 15.47 毫秒、积压清空 41.335 秒，写入到清空合计 51.476 秒，满足 1 分钟目标。新增 `stage7:verify:local` 后端总验收入口，每次创建、迁移并初始化独立临时数据库，顺序执行类型检查、165 个自动化用例及阶段 1～7A 数据库闭环，成功或失败后均删除临时库；因此可与本机长期运行的 API 和各 Worker 并存，不会抢占验收夹具或改动现有业务数据。本轮整套验收已连续通过。
