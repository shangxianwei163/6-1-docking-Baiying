...
阶段 7 尚余真实环境项目：ERP/CRM/百应三方契约，真实百应 `429/5xx` 与网络故障，阿里云 OSS/KMS 和生产 SSO/WAF 安全演练，包含供应商月结的完整账务核对，以及单影楼小流量灰度。现有本地故障注入、重放、SSRF、脱敏、签名 URL、冻结/实扣/释放/超额能力已经纳入总验收，但在真实外部资源到位前不提前勾选对应整项。
