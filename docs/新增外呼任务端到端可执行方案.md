...
> 文档状态：执行中；阶段 0、1、2A、3A、4A、6A、6B 与阶段 5/7 本地内核已完成，真实三方联调、阿里云落地和灰度待执行
> 版本：v1.1
> 编制日期：2026-09-06  
> 最近核验：2026-09-08
...
- 回调签名密钥引用。
- 端点状态为启用。
- URL 为运营端预先固定的 HTTP 或 HTTPS 完整地址，不允许凭证、查询串、片段或投递时跳转。
...
3. 可用余额不足时返回 `INSUFFICIENT_BALANCE` 和公开的 `recharge_qr_code_url`，不得创建任务或调用百应；ERP/CRM 可直接展示该图片供用户扫码充值。
...
补充规则：`MAPPING_VALUE_INVALID` 仍适用于 v1 单任务接口；v2 批次接口将客户级映射值错误降级为逐号码过滤，有效号码继续执行。每个被过滤号码通过现有 `OUTBOUND_CALL_RESULT` 完整结构单独回传，原样携带入参 `guid`，沿用 `CALL_FAILED`，以 `status_text=参数错误` 标识，并在 `summary` 返回具体原因。映射未发布、变量规则缺失等批次级配置错误仍返回 `MAPPING_NOT_READY`。
...
采用固定 `Token`、精简录音清单和平台签名下载地址，不直接在回调请求中发送二进制文件：
...
```json
{
  ...
  "Data": {
    ...
    "occurred_at": "2026-09-06T10:31:00+08:00",
    "company_code": "5903679116",
    "task_no": "PT-20260906-00025",
    "recordings": [
      {
        ...
        "expires_at": "2027-03-05T10:31:00+08:00"
      }
    ]
  }
}
...
`Token` 固定为 `^******^`。`guid`、`phone_masked` 和 `recording_url` 必须来自同一条平台通话记录；ERP/CRM 下载 URL 的截止时间与平台内该录音的 `retentionUntil` 一致，支持单段 HTTP Range 在线播放，也可使用 `recording_id` 重新签发。预签名 URL 按持有即授权处理，不写入普通日志；录音到期或删除后，即使 URL 尚在外部系统中也不能继续访问。
...
- customer_charge / platform_rate / platform_cost / profit numeric(18,6)
- supplier_settlement_id nullable
...
- settlement_month unique（Asia/Shanghai 自然月首日）
- supplier_pricing_tier_id + 阶梯名称、范围、单价快照
- task_count / total_billing_minutes
- total_customer_charge / total_platform_cost / total_profit
- source_hash（预览确认与来源漂移检测）
- finalization_idempotency_key unique
- finalization_reason / finalized_by / finalized_at

supplier_settlement_task_item
- settlement_id + task_id primary key
- task_id unique（同一任务只能进入一个月结批次）
- task_no / billing_minutes / customer_charge
- platform_rate / platform_cost / profit
```
...
| 任务名称     | 平台按 CST 日期 + 一个真实数据分类 + 当日五位序号生成，例如 `20260918排挡3周SS100021`；分类取任务受理时数据库快照中按请求顺序出现的第一个有效分类，不再拼接其他分类；整个名称最多 50 个字符，超长分类在平台侧安全截断。百应名称与平台任务名称保持一致。 |
| 使用线路     | 任务线路快照                                                                                                                                                                                                   |
...
- 录音签名 URL 绑定对象、调用方和有效期；运营后台人工链接默认 15 分钟，ERP/CRM 回调与重签链接截止到录音保留期限。
- 录音下载和人工重放必须记录审计。
...
- 海南人像供应价格没有外部查询接口，由运营根据书面确认的报价人工维护完整阶梯；仅允许从未来自然月开始生效，历史版本不可覆盖。
...
- [ ] 配置真实结果与录音回调地址，完成 HTTP/HTTPS、HMAC、ACK 和幂等连通性验证。
...
- [x] 按百应联调环境验证后的字段实现真实创建、导入、执行和任务查询 HTTP 客户端。
- [x] 实现完成通话列表 HTTP 客户端，用于阶段 4B 分页补偿。
- [x] 复用和加固 OAuth Token 单飞刷新及 `40000010` 重试。
- [x] 使用授权测试号码在百应测试公司完成真实创建 → 导入 → 启动联调。
- [x] 实现启动、暂停、恢复、终止真实命令适配器，并在命令响应后查询确认目标状态。
- [x] 使用授权测试号码完成启动、暂停、恢复、终止的真实联调；平台命令幂等由既有数据库验收覆盖。
- [x] 补充 `429/5xx`、超时、Token 失效和响应 Schema 漂移自动化测试。
- [ ] 在百应测试账号受控验证真实 `429`，不以未经授权的突发请求冲击现有账号。
- [x] 在真实客户端中保证请求/响应日志继续执行字段级脱敏，并保留供应商请求 ID。

阶段 3B 接入记录（2026-09-08）：新增 `HttpBaiyingCallJobClient`，按百应现行表单参数接入创建、按 `jobName` 查询、详情查询、客户导入和 `command=1/2/3` 控制接口。平台创建改为手动任务 `callJobType=2`，避免定时任务缺少 `startDate`；线路数组和客户数组按官方 JSON 数组格式提交。真实写操作默认关闭，只有 `BAIYING_WRITE_ENABLED=true` 时真实任务 Worker 与运营命令才会访问写接口；生命周期验收脚本还要求 `BAIYING_LIVE_TEST_CONFIRM=CALL_REAL_TEST_NUMBER` 与明确测试手机号，失败路径会尽力终止已创建任务。

本轮真实只读/故障探针已通过 OAuth 授权查询、供应商 `requestId`、失效 Token 返回 `40000010` 后只刷新一次，以及 1ms 真实网络超时被分类为可重试查询。另执行 25 个受控并发任务查询，全部成功且未触发真实 `429`，说明当前请求量低于账号限流阈值，不能据此宣称供应商限流响应已实测；HTTP `429`、`5xx`、写入超时与无法解析响应的分类已由确定性自动化覆盖，其中写入不确定结果不会自动重放。为避免继续冲击当前真实账号，脚本把单次真实限流探针上限固定为 25。

真实生命周期验收记录（2026-09-08）：公司 `263120` 使用测试话术 `5145720`（“【测试】六加一-2-12周岁-多agent”）、线路 `1290220` 和一个脱敏展示为 `152****2744` 的明确授权测试号码，创建百应任务 `277868620`。导入返回 `total=1`、`successNum=1`、`placeFailNum=0`、`repeatNum=0`；随后启动、暂停、恢复、终止接口均返回成功且携带供应商 `requestId`，详情查询分别确认 `PAUSED` 和最终 `TERMINATED`。本次没有遗留运行中的测试任务。完成通话分页接口也已用该任务执行只读实测，请求 `pageSize=500` 并获得带供应商 `requestId` 的成功空页；百应空页实际返回 `pageNum=0`，客户端仅在 `total=0/pages=0/list=[]` 时安全规范化为请求页码。阶段 3B 仅剩真实 `429` 响应观测，退出标准尚未全部满足。

```bash
# 真实只读授权、Token 失效和超时探针；不会创建任务或拨号
...
### 阶段 4B：真实百应回调与分页补偿（代码与本地闭环已完成，外部回调验收中）

- [ ] 在阿里云测试入口配置百应回调，验证 HTTPS、WAF/IP、真实报文大小、字段类型和重试行为。
- [x] 使用阶段 3B 的真实客户端实现“查询已完成任务通话列表”分页补偿，每页不超过 500 条。
- [x] 对比百应与平台 `callInstanceId`，补录漏回调；连续两轮数量稳定且无待处理 Inbox 后结束对账。
- [x] 对 15 分钟仍不一致的任务告警并进入人工检查，验证修复后的 Inbox/补偿重放。

阶段 4B 实现记录（2026-09-08）：新增 `0019_stage4b_reconciliation`，为 Callback Inbox 增加 `companyId/callJobId/callInstanceId` 检索列，并为每个任务保存供应商数、平台数、待处理 Inbox、稳定轮次、差异起点、失败退避、人工复核和修复审计。Reconciliation Worker 会扫描进行中、对账中及已完成任务，先确认百应任务已完成，再按 500 条逐页读取完成通话；分页漂移、总数变化、跨任务 ID、重复通话 ID 和超过 100 页都会停止本轮并有界重试。缺失通话被规范化为与真实百应回调同构的报文，经加密 Callback Inbox 进入既有幂等、计费和录音发现链路，不绕过原业务处理器。

本地数据库验收覆盖 501 条两页补偿、任务完成事件补偿、两轮稳定、15 分钟人工复核、已完成任务晚启动补扫，以及人工修复原子恢复 Callback Inbox/死信和幂等重复提交；`npm run db:verify:stage4b` 在 PostgreSQL 上通过 2 个隔离任务和 12 条 Inbox 验证并自动清理。异常中心已增加核对列表和“恢复并立即复查”入口，只有 `MANUAL_REVIEW/FAILED` 可操作，原因、操作人、请求 ID 和幂等键写审计。真实只读探针已对百应任务 `277868620` 验证完成通话 URL、OAuth、500 条页大小、成功响应和供应商请求 ID；该已终止任务没有完成通话，因此真实非空分页及真实漏回调补录仍需由下一次明确授权的完成通话任务验证。

```bash
# 完整本地 4A + 4B 数据库闭环，不联网、不拨号
npm run db:migrate
npm run db:verify:stage4b

# 真实完成通话只读探针，不创建任务、不拨号
BAIYING_LIVE_TEST_SCOPE=transport \
BAIYING_LIVE_COMPLETED_CALL_JOB_ID=277868620 \
npm run baiying:verify:live

# 持续分页补偿与两轮稳定核对
npm run stage4b:worker --workspace @outbound/platform-api
```

退出标准：分页补偿、核对与人工修复的代码和本地闭环已满足；整体阶段仍等待百应向阿里云入口投递至少一组真实任务/通话完成回调，并用真实非空完成通话页确认最终数量稳定一致。

### 阶段 5：录音归档与 ERP/CRM 回传（6～8 个工作日）

- [ ] 接入阿里云 OSS 私有 Bucket、生命周期和 SSE-KMS 服务端加密。
- [x] 实现安全流式下载、域名/IP/重定向校验。
- [x] 实现 SHA-256、大小、类型和对象元数据。
- [x] 实现结果批次事件、任务完成事件和录音清单事件。
- [x] 实现出站 HMAC、投递重试、死信和人工重放。
- [x] 实现分级有效期的签名下载 URL 与重新签发。
- [x] 实现发现/归档/回传三个录音计数。

退出标准：百应临时 URL 过期后，授权用户仍能从平台获取归档录音；接收方停机不会丢事件。

阶段 5A 完成记录（2026-09-06）：新增录音归档 Worker、PostgreSQL `FOR UPDATE SKIP LOCKED` 租约领取、有界重试、死信和人工原位重放闭环；新增仅供开发/测试的本地私有对象存储和 `recording.mock.invalid` 确定性录音源，生产环境明确禁止启用。真实 HTTP 下载器只接受精确 HTTPS 域名允许列表，DNS 全地址与每次重定向均重新校验，并把已校验公网 IP 固定到 TLS 请求，拒绝内网、环回、链路本地、云元数据、IPv4 映射 IPv6 和组播地址。归档采用流式文件头/MIME/长度/100 MiB 上限校验，边传输边计算 SHA-256，通过同目录临时文件原子落盘；成功后保存大小、类型、哈希和默认 180 天保留期限，并更新发现/归档/回传计数。本地运营接口可为授权操作人重新签发默认 15 分钟下载地址，签名绑定录音 UUID、不可逆调用方令牌和有效期，不暴露 Bucket/对象键，签发与打开均留审计。0012 迁移为录音资产增加可用时间、租约、死信时间和保留期限。`npm run db:verify:stage5a` 会创建隔离模拟任务，验证“重试 → 死信 → 人工重放 → 归档成功 → 签名下载 → 死信自动解决”，全程不联网并自动清理业务夹具与临时文件。

阶段 5B 本地内核完成记录（2026-09-06，2026-09-17 更新录音链接策略）：通话结果和任务完成事件与 Callback 业务更新在同一事务写入 Outbox；录音归档成功后写入不含下载地址的内部事件，投影时才为任务所属集成客户端生成截止到录音 `retentionUntil` 的签名 URL，并固化为不可变 `delivery_event`。内容接口支持单段 HTTP Range 和 `206 Partial Content` 在线播放；运营后台人工签发仍保持默认 15 分钟。投影器复用任务创建时固化的端点版本、结果/录音 URL 和签名密钥引用；出站 HMAC 严格绑定 `POST`、回调路径、毫秒时间戳、`eventId` 和原始 Body SHA-256。投递采用 `FOR UPDATE SKIP LOCKED` 租约、八档重试节奏、每次独立 `delivery_attempt`、大多数 `4xx` 永久失败、`408/429/5xx` 与网络异常重试、耗尽后死信；任意 `2xx` 均 ACK 成功。人工重放沿用原 `eventId` 和原 Body，保留累计 attempt 编号，并通过 0013 新增的“本轮尝试数”重新获得一轮重试预算，成功后自动解决死信。新增 ERP/CRM HMAC 鉴权的录音 URL 重签接口，录音所有权绑定任务的 `integration_client_id + source_system`，越权统一返回不存在；0014 将重签响应按客户端和幂等键哈希持久化至少 7 天，同键重试返回首次 URL，同键跨录音复用返回 `IDEMPOTENCY_CONFLICT`。重签地址同样截止到录音 `retentionUntil`；已删除或已过保留期的录音不能重签。

`npm run db:verify:stage5b` 会创建隔离任务并使用严格零网络接收器验证 4 类事件和 6 次投递尝试：启动事件成功、通话结果 503 后重试成功、完成事件成功、录音事件 400 死信后人工重放成功；验证两次尝试的 `eventId` 与 Body 完全相同、签名可验、签名 URL 可读取归档对象，且任务最终状态为结果/录音均已回传、录音计数为 `1 / 1 / 1`。`npm run stage5b:worker:local` 只接受 `erp.mock.invalid` / `crm.mock.invalid`，不创建网络连接，且生产环境启动会被明确拒绝。

阶段 5 尚余阿里云落地项：私有 OSS/SSE-KMS、KMS 密钥提供器、真实百应录音域名允许列表，以及把已实现的 HTTP/HTTPS 网络投递适配器接入生产 Worker。当前不会把本地对象存储、模拟录音源或零网络接收器接入生产启动路径；这些外部条件未具备前，阶段 5 不标记整体完成。
...
### 阶段 6B：配置、账务与异常运营接真（已完成）

- [x] 影楼管理接真实 API，并实现新增、修改、启停的审计闭环。
- [x] 充值记录接账户账本，补齐凭证元数据、退款和调整审批；6B-1 完成账本查询、幂等充值和凭证元数据，6B-2 完成退款/补账/冲减申请与双人复核。
- [x] 话费设置改为数据库版本发布；影楼客户价格与运营人工维护的海南供应价格均直接读写 PostgreSQL，不再使用 `localStorage` 或假设存在供应商价格接口。
- [x] 总览指标和接口日志改为数据库聚合；6B-3A 已完成任务、资金、积压、死信、审批、呼叫趋势和四类接口链路的只读聚合。
- [x] 操作日志改为 PostgreSQL 审计事件聚合，并在服务端统一脱敏敏感详情字段。
- [x] 增加任务控制、安全失败重试、死信查看、人工重放和忽略页面；6B-3B 已完成本地零网络闭环。
- [x] 增加只生成脱敏模拟报文的回调测试工具，并与真实百应测试环境分级隔离。
- [x] 增加运营管理员登录、HttpOnly 签名会话、退出登录和受保护后台 API；百应回调入口保持无运营会话依赖。
- [x] 删除只供旧总览与价格演示使用的静态任务、影楼和流水数组。

6B-1 完成记录（2026-09-06）：新增真实影楼分页/创建/修改/启停接口、不可变账户流水与线下充值接口，以及客户价格预览/版本发布接口。新增影楼自动生成 `YL-YYYYMM-NNNN` 编号，联系人电话加密保存并脱敏读取；充值在账户行锁内同步写余额、账本和审计，支持幂等重放；统一和单影楼价格均先预览再发布，可立即或次日零点生效，旧任务继续使用原快照。

影楼回传端点维护补充记录（2026-09-07，2026-09-09 更新）：新增和编辑影楼弹窗均可配置 ERP 结果回传、ERP 录音回传、CRM 结果回传、CRM 录音回传四个 HTTP 或 HTTPS 地址。每个来源的结果与录音地址必须成对填写；保存时按来源生成新的不可变 `DRAFT` 端点版本并归档旧草稿，未配置 HMAC/KMS 密钥前不允许进入真实回传链路。任务受理继续冻结当时启用的端点版本，后续修改不会改变历史任务。因三个系统属于同一公司，不另设传输模式字段、网关模式或应用层加密；HTTP 与 HTTPS 共用同一 HMAC、幂等、重试和审计链路。

海南供应价格维护补充记录（2026-09-07）：确认海南人像没有可供平台读取的价格接口，供应成本阶梯改为运营后台人工维护。每次发布必须提交覆盖全部分钟区间的完整阶梯，先由服务端预览校验，再预约到上海时区未来自然月首日生效；当前与历史月份继续使用原版本，尚未生效的预约可被整组替换。系统保存发布人、原因、完整阶梯及替换数量到审计日志，月结仍按对应月份唯一有效的供应价格版本计算，不依赖百应线路或价格接口。

本地验收已覆盖影楼生命周期、MC code 唯一性、电话密文、充值幂等与凭证元数据、即时/预约价格解析、审计日志，以及三张页面的真实数据、表单安全提示和只读价格预览。数据库验收脚本自动清理隔离夹具；UI 回归不会执行任何写操作。

6B-2 完成记录（2026-09-06）：新增 `account_adjustment_request` 审批单、退款/补账/冲减申请接口、独立批准/拒绝接口和真实操作日志查询接口。申请人不能复核自己的申请；批准事务会锁定审批单和影楼账户，重新检查可用余额，只生成一条唯一 `REFUND` 或 `ADJUSTMENT` 账本流水；拒绝只记录决策，不改变余额。申请幂等键、申请号、审批状态约束、账本关联唯一性和完整审计均由数据库与服务共同保证；0011 已在独立临时库验证全量迁移与开发回滚。

本地可执行 `npm run db:verify:stage6b2` 验证双人分离、重复请求/重复决策、余额保护、唯一入账、拒绝不入账及嵌套敏感字段脱敏；执行 `npm run stage6b2:verify:ui` 验证审批表单、复核决策入口与真实审计详情。浏览器验收中的待复核申请由只读网络夹具提供，不会产生真实资金写入。运营 API 现要求有效的 HttpOnly 签名会话，并校验浏览器提交的 `X-Actor-Id` 必须与会话账号一致；阿里云生产环境仍需接入 SSO/RBAC 或由可信身份网关注入操作人，不能把当前单管理员本地方案视为生产身份体系。

6B-3A 完成记录（2026-09-06）：新增 `GET /api/v1/operations-overview`，按 Asia/Shanghai 当日口径聚合任务、执行中号码、客户话费、余额/冻结、Callback Inbox、Outbox、投递失败、死信和待复核资金单，并从 `call_instance` 生成逐小时真实呼叫与接听趋势。新增 `GET /api/v1/integration-logs`，以只读联合视图统一展示 ERP/CRM 任务受理、平台到百应的任务编排、百应回调入站及平台到 ERP/CRM 的投递尝试，不另建重复日志账本；支持系统、方向、状态、关键词和分页筛选。

接口日志不会读取或返回 Callback Inbox 的加密原文、投递目标 URL 或完整业务载荷。请求/响应快照、异常文本和嵌套 Header 在服务端统一进行敏感键、Bearer、手机号脱敏后才进入共享 Schema。执行 `npm run db:verify:stage6b3` 可用隔离夹具验证积压阈值、死信关注项、统一链路和递归脱敏，夹具结束后自动清理；执行 `npm run stage6b3:verify:ui` 验证真实总览、真实接口日志、详情弹窗和移动端布局。任务控制、失败重试和死信人工重放已转入下方 6B-3B 完成记录，剩余回调测试工具进入 6B-3C，仍不需要等待真实 ERP/CRM 凭证。

6B-3B 完成记录（2026-09-06）：新增运营任务 `PAUSE`、`RESUME`、`TERMINATE` 命令接口和失败任务安全重试接口。命令先写 `PENDING` 操作、原因、操作人和幂等键，再等待供应商适配器确认；本地适配器严格零网络、零拨号，生产环境不会加载它。终止仅在确认后进入终态并释放剩余冻结，迟到通话仍由既有 Callback 计费链路按真实结果补扣。失败重试只开放可证明安全的创建结果恢复和启动结果未知恢复；部分导入、已产生通话或资金/供应商状态不确定时明确拒绝盲目重放。

新增“异常中心”和 `GET /api/v1/dead-letters`、`POST /api/v1/dead-letters/{id}/replay`、`POST /api/v1/dead-letters/{id}/ignore`。重放会原位恢复 Outbox、Callback、录音或投递记录，不复制业务事件；Outbox/Callback Worker 成功消费后自动把 `REPLAYING` 更新为 `RESOLVED`。详情只返回递归脱敏摘要，所有处置均要求原因、幂等键和操作人审计。人工安全重试支持同一任务的多次冻结/释放周期，每个周期生成独立不可变账本业务键。执行 `npm run db:verify:stage6b3b` 可验证暂停/恢复/终止、命令幂等、终止结算、安全重试重新冻结、不安全重试拦截、Outbox/Callback 重放自动解决、忽略和脱敏，并在结束后自动清理隔离夹具；执行 `npm run stage6b3b:verify:ui` 可验证任务确认弹窗、异常处置、脱敏详情和 390px 移动端布局。

6B-3C 完成记录（2026-09-06）：新增“回调测试”运营入口和 `POST /api/v1/callback-previews`。该接口只接受固定的 `SAFE_PREVIEW` 环境、ERP/CRM 标识、三种事件类型和 1～3 条示例数量，不接受回调 URL、密钥或任意 Header；生成内容使用全新 UUID、`MC-SAFE-PREVIEW`、掩码手机号与 `example.invalid` 录音地址。返回结果明确携带 `deliveryAttempted=false`、`networkAccess=DISABLED`、`destination=null` 和不可验签占位值，服务实现不依赖 HTTP 客户端、端点仓库、投递队列或签名密钥，因此在阿里云生产部署后也不能借此工具误投真实系统。

页面同时展示“安全预览 / 真实联调 / 生产投递”三级边界：目前只开放安全预览；取得 ERP/CRM 测试地址与测试密钥后，应新增独立的受控联调服务，而不是给预览接口增加 URL 或真实签名能力。旧 `views.tsx` 中已被数据库控制台替代的总览、影楼、充值、价格和日志演示实现整体删除，旧静态任务、影楼、流水、浏览器价格存储与演示供应商阶梯随之移除；话术和线路的影楼绑定改为读取真实 `GET /api/v1/studios` 数据。执行 `npm run stage6b3c:verify` 可依次验证静态演示数据退场、严格输入契约、三类事件 Schema、脱敏、零投递能力、无 URL/密钥控件、零外部网络请求、模拟报文切换及 390px 布局。

运营登录补充记录（2026-09-08）：新增 `POST/GET/DELETE /api/v1/operator-session`，账号密码只提交给 API，浏览器仅保存 HttpOnly、SameSite 会话 Cookie，不把密码或会话令牌写入前端存储。后端自动化覆盖正确/错误凭据、篡改与过期 Token、多会话、退出、操作人不一致拦截，以及两个百应回调入口无需运营登录仍可接收。`npm run operator:verify:ui` 覆盖错误登录、正确登录、同浏览器上下文会话共享、退出失效和 390px 布局；原有 UI 脚本统一通过共享认证工具登录。

退出标准：**阶段 6B 已满足**。刷新浏览器或多管理员操作不会丢配置，所有运营页面数字可由数据库、账本与审计日志复核；回调预览只生成合成报文且不具备网络投递能力。

### 阶段 7：全链路测试与试运行（7～10 个工作日）

- [ ] ERP、CRM、百应三方契约测试。
- [x] 10,000 条任务容量和回调突发压测。
- [x] 余额并发、重复请求、重复回调和重复命令测试。
- [ ] 网络超时、百应 `429/5xx`、对象存储故障、ERP/CRM 停机演练。
- [ ] 安全测试：越权、重放、SSRF、敏感日志、签名 URL。
- [x] 账务核对：冻结、实扣、释放、超额和月结（本地真实 PostgreSQL 闭环）。
- [ ] 选 1 家影楼、1 个来源小流量灰度。
- [ ] 观察至少一个完整任务周期后逐步扩大。

阶段 7A 完成记录（2026-09-06）：新增 `db:verify:stage7a`，在真实本地 PostgreSQL、零外部网络条件下使用独立影楼、客户端和队列完成容量与竞争验收。10,000 位客户、每位 20 个业务字段经真实 HTTP 入口受理并原子写入任务、客户密文、冻结、账本、幂等记录和 Outbox，10,001 条被共享契约拒绝；零网络百应适配器完成 10,000 条创建、导入和启动。20 个同幂等键并发请求仅生成 1 个任务和 1 笔冻结；50 个独立任务在仅够 25 个任务的余额下严格得到 25 个成功和 25 个余额不足，账户可用余额保持为 0 而未超卖。10,000 个回调写入 Callback Inbox 后由 10 个 Worker 清空，重复 16 次不新增记录。

本机本轮实测：10,000 条受理 3.662 秒、编排 1.358 秒；普通任务并发受理 P95 1.963 秒；回调写入 10.141 秒、单条写入 P95 15.47 毫秒、积压清空 41.335 秒，写入到清空合计 51.476 秒，满足 1 分钟目标。新增 `stage7:verify:local` 后端总验收入口，每次创建、迁移并初始化独立临时数据库，顺序执行类型检查、197 个自动化用例及阶段 1～7A 数据库闭环，成功或失败后均删除临时库；因此可与本机长期运行的 API 和各 Worker 并存，不会抢占验收夹具或改动现有业务数据。

阶段 7B 完成记录（2026-09-06）：新增供应商月结预览与封账服务、`supplier_monthly_settlement` 不可变批次、逐任务成本快照及迁移 0015。预览按 `Asia/Shanghai` 自然月汇总客户账务已完成任务，以供应商阶梯下界包含、上界不包含的规则解析当月统一成本价；任务详情在封账前动态展示“暂估”平台单价、成本和收益，封账后固化为“最终”，不修改客户单价或客户已扣话费。封账要求月份已经结束、预览 `sourceHash` 未过期、所有任务的通话分钟/话费/账本实扣/冻结捕获与释放均守恒，使用 PostgreSQL 事务咨询锁和 UUID 幂等键保证并发只生成一个批次，并写操作审计。已结束但没有结算任务的月份不要求供应价格，页面显示“无需封账”且后端拒绝生成金额为零的空月结单；存在任务时仍严格校验供应阶梯。封账后发现迟到任务或价格来源变化时不篡改历史批次，而返回 `FINALIZED_SOURCE_DRIFT` 阻断状态供财务处置。“话费设置”页已增加月份选择、四方对账、差异清单、数据指纹和不可逆封账二次确认，当前月或核对不平时不开放提交。

`db:verify:stage7b` 已在真实本地 PostgreSQL、零外部网络下验证 10,000 分钟恰好切换成长阶梯：2 个任务的客户话费保持 ¥4,800.000000，平台成本 ¥1,800.000000、收益 ¥3,000.000000；同时覆盖当前月提前封账拒绝、过期预览拒绝、人为账务差异阻断、并发及重复封账幂等、唯一审计、任务暂估/最终值展示、最终值回写和封账后迟到任务告警。夹具使用独立影楼、独立队列与仅供测试的 2001-01 历史月份，结束后自动清理；`stage7b:verify:ui` 另行覆盖前端预览、确认、封账请求契约、完成态和移动端布局。阶段 7 后端总验收现包含 43 个测试文件、197 个自动化用例和阶段 7B 数据库闭环。

本地自动化状态（2026-09-08）：`npm run lint`、`npm run build`、`npm run backend:typecheck`、`npm run backend:test`、`npm run operator:verify:api`、`npm run ui:verify:local` 和 `npm run stage7:verify:local` 均通过。UI 总验收串行执行 11 组浏览器回归，覆盖运营登录、任务、配置、账务、审批、总览、接口日志、任务控制、死信、回调预览、月结、价格、线路和弹窗布局；运行时要求本地前端、API 与 PostgreSQL 已启动。最新隔离库容量验收中，10,000 条受理 4.259 秒、编排 1.913 秒、普通并发受理 P95 1.820 秒，10,000 个回调写入与清空共 52.120 秒；临时数据库在通过后已自动删除。原有 18 条 UI lint 问题已通过语义化元素、外部状态订阅和显式图表键类型归一化清零，未增加规则豁免。

阶段 7 尚余真实环境项目：ERP/CRM/百应三方契约，真实百应 `429/5xx` 与网络故障，阿里云 OSS/KMS 和生产 SSO/WAF 安全演练，以及单影楼小流量灰度。现有本地故障注入、重放、SSRF、脱敏、签名 URL 和完整账务能力已经纳入总验收，但在真实外部资源到位前不提前勾选对应外部整项。
...
- [x] 手机号规范化、重复判断和 10,000 条边界。
- [x] 统一价格与单影楼价格版本解析。
- [x] 冻结金额十进制计算。
- [x] `duration=0/1/59/60/61` 的计费分钟。
- [x] 多分类同话术同线路通过。
- [x] 多分类跨话术或跨线路拒绝。
- [x] ERP/CRM 字段选择、转换、默认值和 BLOCK 规则。
- [x] 幂等键同报文重放与不同报文冲突。
- [x] 回调状态单调更新和账本唯一键。
- [x] 录音 URL SSRF 校验。
...
- [x] 账户并发冻结不会超卖余额。
- [x] 任务、冻结、Outbox、幂等响应同事务提交。
- [x] 百应创建成功、导入成功、启动成功（本地零网络适配器）。
- [x] 百应真实测试公司完成单号码创建、导入、启动、暂停、恢复和终止，最终状态为已终止。
- [x] 百应创建明确失败（本地故障注入）。
- [x] 百应创建超时但实际成功，可通过任务名称找回（本地故障注入）。
- [x] 百应导入部分失败，不启动并释放冻结（本地故障注入）。
- [x] 启动超时后查询已处于进行中，不重复启动（本地故障注入）。
- [x] 百应回调重复 16 次只结算一次（本地真实 PostgreSQL 模拟回调）。
- [x] 百应回调乱序仍得到正确终态（本地真实 PostgreSQL 模拟回调）。
- [x] 对象存储失败后重试成功（本地私有对象存储故障注入）。
- [x] ERP/CRM 返回 `500/429/400` 的分类和死信行为正确。
