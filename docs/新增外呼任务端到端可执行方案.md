...
采用固定 `Token`、精简录音清单和平台短期签名下载地址，不直接在回调请求中发送二进制文件：
    ...
    "recordings": [
      {
        ...
        "expires_at": "2026-09-06T10:46:00+08:00"
      }
    ]
  }
}
...
`Token` 固定为 `^******^`。`guid`、`phone_masked` 和 `recording_url` 必须来自同一条平台通话记录；下载 URL 默认有效 15 分钟，可使用 `recording_id` 重新签发。预签名 URL 按持有即授权处理，不写入普通日志。
...
阶段 5B 本地内核完成记录（2026-09-06）：通话结果和任务完成事件与 Callback 业务更新在同一事务写入 Outbox；录音归档成功后写入不含下载地址的内部事件，投影时才为任务所属集成客户端生成 15 分钟签名 URL，并固化为不可变 `delivery_event`。投影器复用任务创建时固化的端点版本、结果/录音 URL 和签名密钥引用；出站 HMAC 严格绑定 `POST`、回调路径、毫秒时间戳、`eventId` 和原始 Body SHA-256。投递采用 `FOR UPDATE SKIP LOCKED` 租约、八档重试节奏、每次独立 `delivery_attempt`、大多数 `4xx` 永久失败、`408/429/5xx` 与网络异常重试、耗尽后死信；任意 `2xx` 均 ACK 成功。人工重放沿用原 `eventId` 和原 Body，保留累计 attempt 编号，并通过 0013 新增的“本轮尝试数”重新获得一轮重试预算，成功后自动解决死信。新增 ERP/CRM HMAC 鉴权的录音 URL 重签接口，录音所有权绑定任务的 `integration_client_id + source_system`，越权统一返回不存在；0014 将重签响应按客户端和幂等键哈希持久化至少 7 天，同键重试返回首次 URL，同键跨录音复用返回 `IDEMPOTENCY_CONFLICT`，URL 过期后调用方需使用新幂等键。

`npm run db:verify:stage5b` 会创建隔离任务并使用严格零网络接收器验证 4 类事件和 6 次投递尝试：启动事件成功、通话结果 503 后重试成功、完成事件成功、录音事件 400 死信后人工重放成功；验证两次尝试的 `eventId` 与 Body 完全相同、签名可验、短期 URL 可读取归档对象，且任务最终状态为结果/录音均已回传、录音计数为 `1 / 1 / 1`。`npm run stage5b:worker:local` 只接受 `erp.mock.invalid` / `crm.mock.invalid`，不创建网络连接，且生产环境启动会被明确拒绝。
