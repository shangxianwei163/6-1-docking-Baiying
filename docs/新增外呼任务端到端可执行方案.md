...
阶段 7B 完成记录（2026-09-06）：新增供应商月结预览与封账服务、`supplier_monthly_settlement` 不可变批次、逐任务成本快照及迁移 0015。预览按 `Asia/Shanghai` 自然月汇总客户账务已完成任务，以供应商阶梯下界包含、上界不包含的规则解析当月统一成本价；任务详情在封账前动态展示“暂估”平台单价、成本和收益，封账后固化为“最终”，不修改客户单价或客户已扣话费。封账要求月份已经结束、预览 `sourceHash` 未过期、所有任务的通话分钟/话费/账本实扣/冻结捕获与释放均守恒，使用 PostgreSQL 事务咨询锁和 UUID 幂等键保证并发只生成一个批次，并写操作审计。封账后发现迟到任务或价格来源变化时不篡改历史批次，而返回 `FINALIZED_SOURCE_DRIFT` 阻断状态供财务处置。“话费设置”页已增加月份选择、四方对账、差异清单、数据指纹和不可逆封账二次确认，当前月或核对不平时不开放提交。
