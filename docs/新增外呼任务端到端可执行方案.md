...
接口日志不会读取或返回 Callback Inbox 的加密原文、投递目标 URL 或完整业务载荷。请求/响应快照、异常文本和嵌套 Header 在服务端统一进行敏感键、Bearer、手机号脱敏后才进入共享 Schema。执行 `npm run db:verify:stage6b3` 可用隔离夹具验证积压阈值、死信关注项、统一链路和递归脱敏，夹具结束后自动清理；执行 `npm run stage6b3:verify:ui` 验证真实总览、真实接口日志、详情弹窗和移动端布局。回调测试、暂停/恢复/终止、重试和死信人工重放仍留在 6B-3B，不需要等待真实 ERP/CRM 凭证即可继续开发。
