...
```text
callJobName = PT-{taskNo}-{shortHash}
...
### 阶段 2：创建接口与预检（5～7 个工作日）

- [ ] 实现 HMAC、时间戳、Nonce、权限和限流。
- [ ] 实现幂等键及同键不同报文冲突。
- [ ] 实现请求 Schema 和 10,000 条上限。
- [ ] 实现 MC code、端点、价格、余额、分类、话术、线路、映射预检。
- [ ] 实现全量客户变量转换和错误明细。
- [ ] 实现任务编号、配置快照和原子资金冻结。
- [ ] 实现 `POST/GET task` 与通话查询接口。
- [ ] 为所有同步拒绝场景编写契约测试。

退出标准：不调用百应也能完整验证和可靠受理任务；重复请求只生成一条任务和一笔冻结。

### 阶段 3：百应任务编排（5～7 个工作日）

- [ ] 实现 `BaiyingCallJobClient` 的创建、导入、执行、任务查询、完成通话列表。
- [ ] 复用和加固 OAuth Token 单飞刷新及 `40000010` 重试。
- [ ] 实现任务编排状态机和 Worker。
- [ ] 实现确定性 `callJobName` 及创建结果未知的查询恢复。
- [ ] 实现严格全量导入判定、失败终止和冻结释放。
- [ ] 实现启动、暂停、恢复、终止命令。
- [ ] 保存每次百应调用的脱敏请求、响应、耗时和请求 ID。

退出标准：测试环境可完成创建 → 导入 → 启动，且每个故障点都能恢复或形成明确死信。

### 阶段 4：回调、状态与计费（6～8 个工作日）

- [ ] 将现有单次呼叫回调改为先保存原始正文再 ACK。
- [ ] 接入 `CALL_INSTANCE_RESULT` 和 `JOB_INFO_RESULT`。
- [ ] 实现回调重复、乱序和未知任务处理。
- [ ] 通过 `sx_platform_item_id` 关联客户明细，手机号作为后备匹配。
- [ ] 实现逐通话计费、冻结捕获、差额释放和超额扣款。
- [ ] 实现任务完成后的百应分页对账。
- [ ] 实现执行状态与页面状态映射。

退出标准：重复回调不会重复计费；漏回调可通过补偿查询补齐；任务状态可解释。
...
### 阶段 6：页面接真与运营能力（5～7 个工作日）

- [ ] 影楼管理接真实 API。
- [ ] 话费设置改为版本发布。
- [ ] 任务列表和详情接真实任务、计费、录音和回传状态。
- [ ] 增加回调测试、任务控制、重试和死信页面。
- [ ] 增加金额暂估/最终标识和失败原因提示。
- [ ] 去除对应静态数组和 `localStorage` 业务数据。

退出标准：刷新浏览器或多管理员操作不会丢配置，页面数字可由数据库和账本复核。
