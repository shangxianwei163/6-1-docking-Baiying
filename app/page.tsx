...
import { BadgeDollarSign, BellRing, CircleDollarSign, FileCog, Gauge, Landmark, PhoneCall, ReceiptText, Settings2 } from 'lucide-react';
import { AuditLogView, BaiyingBillView, MappingView, OverviewView, RateSettingView, RechargeView, StoreBillView, StudioView, TaskView } from '@/components/platform/views';

export type PlatformSection = '总览' | '影楼管理' | '呼叫任务' | '字段映射' | '百应账单' | '门店账单' | '充值记录' | '话费设置' | '操作日志';

const navigation: Array<{ label: PlatformSection; icon: typeof Gauge; hint?: string }> = [
  { label: '总览', icon: Gauge }, { label: '影楼管理', icon: Settings2 }, { label: '呼叫任务', icon: PhoneCall, hint: '5' },
  { label: '字段映射', icon: FileCog }, { label: '百应账单', icon: ReceiptText }, { label: '门店账单', icon: Landmark },
  { label: '充值记录', icon: CircleDollarSign }, { label: '话费设置', icon: BadgeDollarSign }, { label: '操作日志', icon: BellRing, hint: '3' },
];

const pageMap: Record<PlatformSection, React.ReactNode> = {
  总览: <OverviewView />, 影楼管理: <StudioView />, 呼叫任务: <TaskView />, 字段映射: <MappingView />,
  百应账单: <BaiyingBillView />, 门店账单: <StoreBillView />, 充值记录: <RechargeView />,
  话费设置: <RateSettingView />, 操作日志: <AuditLogView />,
};
...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#f5f5f1] text-[#182a27]"><div className="app-shell">
    <aside className="sidebar">
      <div className="brand-block"><div className="brand-kicker"><span className="signal-dot" />INTELLIGENT OUTBOUND</div><h1>6+1 调度台</h1><p>ERP / CRM / 百应 · 统一运营后台</p></div>
