...
export function transformMappedValue(input: {
  ...
  if (empty)
    return handleEmpty(
      input.rule.baiyingVariableName,
      input.rule.emptyPolicy,
      input.rule.defaultValue,
      rawValue,
      field,
    );
  ...
  try {
    ...
    if (
      input.rule.emptyPolicy === 'DEFAULT' &&
      input.rule.defaultValue !== null
    )
      return input.rule.defaultValue;
    throw new MappingValueError(
      input.rule.baiyingVariableName,
...
function handleEmpty(
  ...
  throw new MappingValueError(
    variableName,
    field ? `来源字段 ${field} 为空` : '当前来源系统未配置取值字段',
    rawValue,
  );
}
...
function applyTransform(value: string, config: TransformConfig): string {
  ...
  if (config.type === 'DATE') {
    ...
    if (config.outputFormat === 'YYYY年MM月DD日')
